...
export default async function DashboardPage() {
  ...
  // ── INDIVIDUAL view ──────────────────────────────────────────────────────────
  // Individual users go directly to their self-worker's cert management page,
  // which is the same UI as the Business worker detail page.
  if (accountType === 'individual') {
    const { data: workers } = await supabase
      .from('workers').select('id').eq('user_id', user?.id ?? '').limit(1)
    const selfWorker = workers?.[0] ?? null
    if (selfWorker) redirect(`/dashboard/workers/${selfWorker.id}`)
    // Edge case: self-worker was never created — send back to onboarding to recreate it.
    redirect('/onboarding')
  }
